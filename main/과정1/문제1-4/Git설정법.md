...
## 🧹 Git 저장소 초기화: `.git` 폴더 완전히 제거 & 재설정

Git 저장소를 완전히 초기화하고 싶을 때는 `.git/` 폴더를 삭제하면 됩니다.  
이는 Git이 해당 디렉토리를 더 이상 저장소로 인식하지 않게 만듭니다.
...
| 방법 | 명령어 / 절차 | 설명 |
|------|----------------|------|
| 터미널에서 삭제 (추천) | `rm -rf .git` | 현재 디렉토리 내 `.git/` 폴더 완전 삭제 |
| GUI에서 삭제 | `Ctrl + H`로 숨김 파일 보기 → `.git` 폴더 삭제 | 마우스로 삭제 가능 (휴지통 비우기까지 해야 완전 제거) |

> **주의:** `rm -rf`는 되돌릴 수 없습니다. 꼭 `git_practice` 폴더 안에서 실행하세요.
...
| 단계 | 명령어 | 설명 |
|------|--------|------|
| 1 | `cd ~/Desktop/git_practice` | 작업 디렉토리로 이동 |
| 2 | `git status` | 현재 Git 저장소 상태 확인 |
| 3 | `git init` | `.git`이 없으면 다시 초기화 (이전 이력은 사라짐) |

> `git init`을 다시 실행하면 새로운 Git 저장소가 설정되며, 기존 커밋 기록은 복구되지 않습니다.
